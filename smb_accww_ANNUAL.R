...
#size category: adult, juvenile_YOY, juvenile_LY
size_cat <- 'juvenile_YOY' 

#boolean to lag AccWW to previous year
lag_year <- TRUE
...
#Annual accww data
accww_dat <- read.csv('annual_accww.csv', stringsAsFactors = TRUE, 
                      na.strings= c("NA", ""))

# add station_name column
names(accww_dat)[1] <- 'station_name'
...
for(i in 1:length(unique(smb_dat$station_name))){
  ...
  for(j in 1:length(unique(smb.i$year))){
    ...
    #total catch for current size category fish at current site/year
...
for(i in 1:nrow(total_cpe)){
  ...
  #pick indexes for qdat corresponding to year and sites
  if(lag_year==TRUE){
    #LAGGED YEAR
    index.i <- which(qdat$station_name==as.vector(sn.i) &
